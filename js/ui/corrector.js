...
const Corrector = (function () {
  ...
  /**
   * @param {Object} respuestaAlumno - { [procesoId]: Array<'CPU'|'IO'|null> },
  ...
  function corregir(respuestaAlumno, procesos, resultado) {
    ...
    const duracionAlumno = Math.max(0, ...procesos.map((p) => (respuestaAlumno[p.id] || []).length));
    const duracionTotal = Math.max(duracionSolucion, duracionAlumno);

    const celdasIncorrectas = [];
    procesos.forEach((proceso) => {
      const respuestaProceso = respuestaAlumno[proceso.id] || [];
      for (let t = 0; t < duracionTotal; t++) {
        const valorAlumno = respuestaProceso[t] || null;
        const valorSolucion = t < duracionSolucion ? datos[proceso.id][t] : null;
        if (valorAlumno !== valorSolucion) celdasIncorrectas.push({ procesoId: proceso.id, instante: t });
      }
    });
